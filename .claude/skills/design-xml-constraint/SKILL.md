---
name: design-xml-constraint
description: Direct entry point for the Android Workflow "Designs → XML → ConstraintLayout Only" command — implement a screen design as an XML layout using one root ConstraintLayout with every view as a direct child and no other ViewGroups, from a screenshot, a Figma frame, and/or a written description. Use when the user explicitly invokes /design-xml-constraint or names this command. Equivalent to Android Workflow → Designs → XML → ConstraintLayout Only, with no mode questions.
---

# Direct Entry: Designs — XML, ConstraintLayout Only
...
1. Read `../android-workflow/commands/design-xml-constraint.md` — resolved against **this skill's own base directory**, which is given to you when the skill is invoked — and follow it completely, from its first step to its last.
...
- Behavior must be identical whether reached from here or from the menu. Launching directly is not a reason to act differently — in particular, it is not a reason to relax the single-ConstraintLayout constraint or to switch to another layout strategy without asking.
